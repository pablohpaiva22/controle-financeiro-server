...
router.post("/", (req, res) => {
  ...
  const sql = `INSERT INTO railway.users (name, email, password) VALUES ('${req.body.user}', '${req.body.email}' ,'${req.body.password}')`;
  ...
  connection.query(
    ...
    function (err) {
      if (err) {
        ...
        res.status(401).send({msg: 'Erro ao acessar o banco de dados'});
        return false;
      }
